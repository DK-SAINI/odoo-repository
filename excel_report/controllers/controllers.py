...
class SaleExcelReportController(http.Controller):
    @http.route(
        ['/sale/excel_report/<model("sale.wizard"):wizard>',],
        type="http",
        auth="user",
        csrf=False,
    )
    def get_sale_excel_report(self, wizard=None, **args):
        ...
        # with the get_excel_report method in the sale.wizard model
        ...
        response = request.make_response(
            ...
            headers=[
                ...
                (
                    "Content-Disposition",
                    content_disposition(
                        "Sales Report in Excel Format" + ".xlsx"
                    ),
                ),
            ],
        )
        ...
        for user in wizard.user_id:
            ...
            sheet.merge_range(
                "A1:E1", "Sales Report in Excel Format", title_style
            )
            ...
            sheet.merge_range(
                "A" + str(row + 1) + ":D" + str(row + 1), "Total", text_style
            )
            # For Total Press "ctrl + shiift + f9"
            sheet.write_formula(
                row, 4, "=SUM(E3:E" + str(row) + ")", number_style
            )
