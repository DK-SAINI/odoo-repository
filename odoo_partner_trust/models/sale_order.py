...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):
        ...
        # is called from the sale.order.wizard transient model and the user have
        # 'Approve sale order of blacklisted customer' access rights, so the sale order can be confirmed
        if (
            self.partner_id.trust_state == "trusted"
            or "approve_blacklisted_customer_order" in self._context
        ):
            ...
            # by inserting the order_id field, later in the sale.order.wizard transient model
            # we can call the action_confirm method to confirm this sales order
            wizard_id = self.env["sale.order.wizard"].create({"order_id": self.id})
            return {
